...
#import <OrderContentItemDto.h>
#import <NomenclatureDto.h>

@implementation OrderContentItemDto

- (instancetype)initWithBondObject:(void*)obj {
  if (self = [super init]) {
    ::OrderContentItem* castObj = (::OrderContentItem*)obj;
...
- (instancetype)initWithByteArray: (uint8_t*)byteArray length:(uint32_t)len {
  ::OrderContentItem obj = CppBondSerializer<::OrderContentItem>::deserializeFromByteArray(byteArray, len);
  return [self initWithBondObject: &obj];
}

- (void*)toBondObject {
  ::OrderContentItem* obj = new OrderContentItem();
...
-(const uint8_t*) toByteArray: (uint32_t*)resultLen {
  auto obj = (::OrderContentItem*)[self toBondObject];
  return CppBondSerializer<::OrderContentItem>::serializeToByteArray(*obj, resultLen);
}
