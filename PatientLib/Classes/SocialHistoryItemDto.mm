...
#import <SocialHistoryItem_reflection.h>
#import <SocialHistoryItemDto.h>
#import <NomenclatureDto.h>

@implementation SocialHistoryItemDto

- (instancetype)initWithBondObject:(void*)obj {
  if (self = [super init]) {
    ::SocialHistoryItem* castObj = (::SocialHistoryItem*)obj;
    ...
    if (!castObj->SocialBehavior.is_nothing() && castObj->SocialBehavior.value().hasvalue()) self.socialBehavior =  [[NSNumber alloc] initWithInt: castObj->SocialBehavior.value().value()];
  }
  return self;
}
...
- (instancetype) initWithJsonDictionary: (NSDictionary*)dictionary {
  if (self = [super init]) {
    ...
    if ([dictionary.allKeys containsObject: @"SocialBehavior" ] && [dictionary objectForKey: @"SocialBehavior"] != (id)[NSNull null]) {
      self.socialBehaviorStringValue = [dictionary objectForKey: @"SocialBehavior"];
    }
  }
  return self;
}
...
- (instancetype)initWithByteArray: (uint8_t*)byteArray length:(uint32_t)len {
  ::SocialHistoryItem obj = CppBondSerializer<::SocialHistoryItem>::deserializeFromByteArray(byteArray, len);
  return [self initWithBondObject: &obj];
}

- (void*)toBondObject {
  ::SocialHistoryItem* obj = new SocialHistoryItem();
  ...
  if (self.socialBehavior != nil) obj->SocialBehavior.set_value().set(static_cast<::SocialBehavior>([self.socialBehavior intValue]));
  return obj;
}

-(const uint8_t*) toByteArray: (uint32_t*)resultLen {
  auto obj = (::SocialHistoryItem*)[self toBondObject];
  return CppBondSerializer<::SocialHistoryItem>::serializeToByteArray(*obj, resultLen);
}
