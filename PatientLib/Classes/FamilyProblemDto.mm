...
#import <FamilyProblem_reflection.h>
#import <FamilyProblemDto.h>
#import <NomenclatureDto.h>

@implementation FamilyProblemDto

- (instancetype)initWithBondObject:(void*)obj {
  if (self = [super init]) {
    ::FamilyProblem* castObj = (::FamilyProblem*)obj;
    ...
    if (!castObj->RelationshipType.is_nothing() && castObj->RelationshipType.value().hasvalue()) self.relationshipType =  [[NSNumber alloc] initWithInt: castObj->RelationshipType.value().value()];
    ...
    if (!castObj->IsDeceased.is_nothing() && castObj->IsDeceased.value().hasvalue()) self.isDeceased =  [[NSNumber alloc] initWithBool: castObj->IsDeceased.value().value()];
  }
  return self;
}
...
- (instancetype) initWithJsonDictionary: (NSDictionary*)dictionary {
  if (self = [super init]) {
    ...
    if ([dictionary.allKeys containsObject: @"RelationshipType" ] && [dictionary objectForKey: @"RelationshipType"] != (id)[NSNull null]) {
      self.relationshipTypeStringValue = [dictionary objectForKey: @"RelationshipType"];
    }
    ...
    if ([dictionary.allKeys containsObject: @"IsDeceased" ] && [dictionary objectForKey: @"IsDeceased"] != (id)[NSNull null]) {
      self.isDeceased = [dictionary objectForKey: @"IsDeceased"];
    }
  }
  return self;
}
...
- (instancetype)initWithByteArray: (uint8_t*)byteArray length:(uint32_t)len {
  ::FamilyProblem obj = CppBondSerializer<::FamilyProblem>::deserializeFromByteArray(byteArray, len);
  return [self initWithBondObject: &obj];
}

- (void*)toBondObject {
  ::FamilyProblem* obj = new FamilyProblem();
  ...
  if (self.relationshipType != nil) obj->RelationshipType.set_value().set(static_cast<::FamilyRelationshipType>([self.relationshipType intValue]));
  if (self.dateOfBirth != nil) obj->DateOfBirth.set_value().set([self.dateOfBirth timeIntervalSince1970]);
  if (self.isDeceased != nil) obj->IsDeceased.set_value().set([self.isDeceased boolValue]);
  return obj;
}

-(const uint8_t*) toByteArray: (uint32_t*)resultLen {
  auto obj = (::FamilyProblem*)[self toBondObject];
  return CppBondSerializer<::FamilyProblem>::serializeToByteArray(*obj, resultLen);
}
